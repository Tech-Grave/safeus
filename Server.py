...
class AboutApp(Resource):
    ...
    def get(self):
        idea_description = "Since we are facing critical situation due to the COVID 19 pandemic. To win against this " \
                           ...
                           "Domains under focused seller: local store, pharmacy, ration shop, milk dairy, " \
                           "drinking water, boutique, bakery, grocery, doctor clinic, etc. " \
...
class BookSlot(Resource):
    ...
    def get(self, email, shopId, note):
        ...
        if isShopTakingBookings is True:
            ...
            """
            isBookingDone = DatabaseHandling.isBookingDone(shopId, email, note)
...
api.add_resource(BookSlot, '/bookslot/<string:email>/<string:shopId>/<string:note>')
