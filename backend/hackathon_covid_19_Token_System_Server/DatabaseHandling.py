...
def isBookingDone(shopId, email, note):
...
def isSlotBookedNotClashing(userEmail, currentIncomingTime, currentOutgoingTime):
    ...
    if data is None:
        ...
        """
        if str(currentIncomingTime) not in eventBusyScheduleList or str(currentOutgoingTime) not in eventBusyScheduleList:
            return True
        else:
            return False
...
def getUserEvents(filter, email):
    ...
    try:
        ...
        myCursor.execute(
            "SELECT `own`.`shop_id`, `own`.`shop_name`, `own`.`owner_name`, `own`.`address`, "
            "date_format(`t`.`date`, '%W %d, %M %y') `event_day`, "
            "TIME_FORMAT(`t`.`incoming_time`, '%H:%i:00') `incoming_time`, "
            "TIME_FORMAT(`t`.`outgoing_time`, '%H:%i:00') `outgoing_time`, `t`.`status` "
            ...
            "ORDER BY `t`.`date` DESC, `t`.`incoming_time` DESC;"
        )
        row_headers = [x[0] for x in myCursor.description]  # this will extract row headers
